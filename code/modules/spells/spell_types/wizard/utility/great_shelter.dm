...
//
// Layout (3x3, door facing caster's direction):
//   [wall] [wall] [wall]
//   [wall] [interior] [wall]
//   [wall] [door] [wall]
//
...
/datum/action/cooldown/spell/great_shelter/cast(atom/cast_on)
	...
	// Build walls around the perimeter, door on the side facing the caster
	for(var/turf/T in range(1, center))
		if(T == center)
			continue
		if(get_dir(center, T) == door_dir)
			// Door tile
	...
	new /obj/structure/bed/rogue/conjured(center)
	new /obj/machinery/light/rogue/hearth/conjured(center)
