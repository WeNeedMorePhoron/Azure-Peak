/datum/telegraphed_strike/spellblade
...
/datum/telegraphed_strike/spellblade/get_strike_weapon(mob/living/carbon/human/H)
	return arcyne_get_weapon(H)

/datum/telegraphed_strike/spellblade/get_strike_damage()
	var/dmg = damage
	if(blade_class == BCLASS_BLUNT)
		dmg = max(0, dmg - blunt_penalty)
	return empowered ? dmg * empowered_mult : dmg

/datum/telegraphed_strike/spellblade/on_antimagic_block(mob/living/L)
	L.visible_message(span_warning("The arcyne force scatters as it nears [L]!"))
	playsound(get_turf(L), 'sound/magic/magic_nulled.ogg', 100)

/datum/telegraphed_strike/spellblade/on_hit_target(mob/living/carbon/human/H, mob/living/L, facing)
	if(!push_dist)
		return
...
/datum/telegraphed_strike/spellblade/on_strike_complete(mob/living/carbon/human/H, hit_count, deflected)
	if(!hit_count)
		return
...
/datum/telegraphed_strike/spellblade/on_impact(mob/living/carbon/human/H, facing, atom/movable/visual)
...
/datum/telegraphed_strike/spellblade/proc/play_impact(turf/center, facing, list/tiles)
	if(blade_class == BCLASS_BLUNT)
...
/datum/action/cooldown/spell/telegraphed_strike/spellblade
	...
	engine_type = /datum/telegraphed_strike/spellblade

/datum/action/cooldown/spell/telegraphed_strike/spellblade/configure_engine(mob/living/carbon/human/H, datum/telegraphed_strike/E)
	var/datum/telegraphed_strike/spellblade/SB = E
	var/datum/intent/active = H.a_intent
	if(active && (active.blade_class == BCLASS_BLUNT || active.blade_class == BCLASS_SMASH))
		SB.blade_class = BCLASS_BLUNT
	else
		SB.blade_class = BCLASS_CUT
	SB.empowered = FALSE
	var/datum/status_effect/buff/arcyne_momentum/M = H.has_status_effect(/datum/status_effect/buff/arcyne_momentum)
	if(M && M.stacks >= SB.momentum_cost)
		M.consume_stacks(SB.momentum_cost)
		SB.empowered = TRUE
		to_chat(H, span_notice("[SB.momentum_cost] momentum released - empowered strike!"))
	return TRUE
