...
/mob/living/carbon/human/death(gibbed, nocutscene = FALSE)
	...
	if(client || mind)
		record_round_statistics(STATS_DEATHS)
		var/area_of_death = lowertext(get_area_name(src))
		if(area_of_death == "wilderness")
			record_round_statistics(STATS_FOREST_DEATHS)
		if(is_noble())
			record_round_statistics(STATS_NOBLE_DEATHS)
		if(ishumannorthern(src))
			record_round_statistics(STATS_HUMEN_DEATHS)
		if(mind)
			if(mind.assigned_role in GLOB.church_positions)
				record_round_statistics(STATS_CLERGY_DEATHS)
			if(mind.has_antag_datum(/datum/antagonist/vampire))
				record_round_statistics(STATS_VAMPIRES_KILLED)
			if(mind.has_antag_datum(/datum/antagonist/zombie))
				record_round_statistics(STATS_DEADITES_KILLED)
			if(mind.has_antag_datum(/datum/antagonist/skeleton) || mind.has_antag_datum(/datum/antagonist/lich))
				record_round_statistics(STATS_SKELETONS_KILLED)
...
/mob/living/carbon/human/gib(no_brain, no_organs, no_bodyparts, safe_gib = FALSE)
	record_round_statistics(STATS_PEOPLE_GIBBED)
