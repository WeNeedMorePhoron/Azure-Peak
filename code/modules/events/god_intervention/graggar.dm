...
/datum/culling_duel/proc/handle_heart_destroyed(which_heart)
	...
	if(loser)
		...
		loser.change_stat(STATKEY_PER, -1)
		loser.change_stat(STATKEY_INT, -1)
...
/datum/culling_duel/proc/process_win(mob/living/winner, mob/living/loser)
	...
	winner.change_stat(STATKEY_PER, 1)
	winner.change_stat(STATKEY_INT, 1)
	...
	ADD_TRAIT(winner, TRAIT_STRENGTH_UNCAPPED, TRAIT_GENERIC)
	to_chat(winner, span_danger("THE DARKSTAR SMILES AT ME. Victory is mine, and with it comes liberated strength of the conquered!"))
