	...
	if(istype(P, /obj/item/inqarticles/bmirror))		
		if((HAS_TRAIT(user, TRAIT_INQUISITION) || HAS_TRAIT(user, TRAIT_PURITAN)))	
			...
			if(I.broken && !I.bloody)
				...
				qdel(I)
				record_round_statistics(STATS_MARQUES_MADE, 2)
				...
				if(no | selfreport)		
					...
					if(!correct)
						if(cursedblood)
							...
							record_round_statistics(STATS_MARQUES_MADE, bonuses)
						...
						record_round_statistics(STATS_MARQUES_MADE, I.marquevalue)
					if(I.paired)	
						qdel(I.paired)	
					qdel(I)
					visible_message(span_warning("[user] sends something."))
