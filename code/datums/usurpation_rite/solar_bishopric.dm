...
<p><b>Realm type if successful:</b> Archbishopric, ruled by an Archbishop.</p>"}
...
/datum/usurpation_rite/solar_bishopric/on_gathering_started()
...
/datum/usurpation_rite/solar_bishopric/on_assent_accepted(mob/living/carbon/human/faithful)
	...
	faithful.visible_message( \
		span_notice("[faithful.real_name] speaks their assent to the Rite of Solar Bishopric."), \
...
/datum/usurpation_rite/solar_bishopric/check_assent_threshold()
	if(get_assent_total() >= BISHOPRIC_REQUIRED_ASSENTS)
		start_contesting()

/datum/usurpation_rite/solar_bishopric/on_contesting_started()
	priority_announce( \
		"[invoker.real_name] has invoked the Rite of Solar Bishopric!\n\n" + \
...
/datum/usurpation_rite/solar_bishopric/get_status_text()
	switch(stage)
		if(RITE_STAGE_GATHERING)
			return "The Rite of Solar Bishopric is underway. [get_assent_total()]/[BISHOPRIC_REQUIRED_ASSENTS] voices have spoken their assent."
		if(RITE_STAGE_CONTESTING)
			return "The faithful have affirmed [invoker?.real_name]'s claim. The Sun's judgment approaches."
	return null

/datum/usurpation_rite/solar_bishopric/get_periodic_announcement()
	switch(stage)
		if(RITE_STAGE_GATHERING)
...
/datum/usurpation_rite/solar_bishopric/proc/is_qualified_faithful(mob/living/carbon/human/user)
	if(user.job in GLOB.church_positions)
		return TRUE
...
/datum/usurpation_rite/solar_bishopric/proc/get_vote_weight(mob/living/carbon/human/faithful)
...
/datum/usurpation_rite/solar_bishopric/proc/get_assent_total()
	var/total = 0
